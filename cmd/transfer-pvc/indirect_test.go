...
func TestHasRcloneSection(t *testing.T) {
	tests := []struct {
		name   string
		config string
		want   bool
...
`,
		...
		{
			name: "BOM-prefixed section",
			config: "\uFEFF[encrypted]\n" +
				"type = crypt\n",
			want: true,
		},
		{
			name: "section with trailing comment",
			config: `[encrypted] # generated by a prior run
type = crypt
`,
			want: true,
		},
		{
			name: "comment mentioning section",
			config: `[remote]
type = s3
# do not add an [encrypted] section here
`,
			want: false,
		},
		{
			name: "setting value mentioning section",
			config: `[remote]
note = [encrypted]
`,
			want: false,
		...
		{
			name: "similarly named section is not a collision",
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := hasRcloneSection([]byte(tt.config), "encrypted"); got != tt.want {
				t.Errorf("hasRcloneSection() = %v, want %v", got, tt.want)
			}
		})
	}
}
