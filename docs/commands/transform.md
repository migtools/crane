...
- **`output/`**: Materialized output after applying kustomization (organized by namespace)
...
```bash
# Using crane apply (recommended — uses embedded kustomize)
crane apply --transform-dir transform --output-dir output

# Or preview with kubectl kustomize (if kubectl is installed)
...
```bash
# Option 1: Use crane apply (no kubectl dependency for this step)
crane apply --transform-dir transform --output-dir output
kubectl apply -f output/output.yaml

# Option 2: Direct apply with kubectl
...
└── resources/                       # Individual resource files
    ...
    └── _cluster/                    # Cluster-scoped resources
        └── ClusterRoleBinding_rbac.authorization.k8s.io_v1_clusterscoped_myapp.yaml
```
...
# Output:
# - input/ (from previous stage or export)
...
# Output:
# - input/ (copied unchanged from previous stage)
...
```bash
# Use crane apply to render and review
crane apply
cat output/output.yaml

# Or view specific resource type
crane apply
grep -A 20 "kind: Deployment" output/output.yaml

# Or use kubectl kustomize directly (if installed)
kubectl kustomize transform/10_KubernetesPlugin/
```
...
```gitignore
# Crane stage output directories (regenerated)
transform/*/output/
...
git diff --staged transform/10_KubernetesPlugin/input/
...
ls -la transform/10_KubernetesPlugin/input/
...
**Solution**:
```bash
# Check input to see what was loaded
ls -la transform/10_KubernetesPlugin/input/
