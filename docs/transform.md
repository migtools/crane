...
└── 10_KubernetesPlugin/
    ...
    ├── new/
    │   └── Build_shipwright.io_v1beta1_default_my-app-build.yaml
    ├── patches/
...
- **`new/`**: New resources generated by a plugin during this stage
- **`patches/`**: Kustomize patches to apply to resources
- **`output/`**: Materialized output after applying kustomization
- **`kustomization.yaml`**: Kustomize configuration file (references `input/` and `new/`)
...
│   ├── new/                 # Plugin-generated resources
...
Contains new resources generated by plugins. These are stored as minimal skeleton files, with additional modifications applied via files in the `patches/` directory.
