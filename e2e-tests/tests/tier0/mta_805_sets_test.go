...
var _ = Describe("Sets resources migration", func() {
	It("[MTA-805] Should migrate ReplicaSet, DaemonSet and StatefulSet as namespace-admin", Label("tier0"), func() {
		...
		daemonSetPattern := filepath.Join(paths.OutputDir, "resources", namespace, "*_DaemonSet_*.yaml")
		...
		replicaSetPattern := filepath.Join(paths.OutputDir, "resources", namespace, "*_ReplicaSet_*.yaml")
		...
		statefulSetPattern := filepath.Join(paths.OutputDir, "resources", namespace, "*_StatefulSet_*.yaml")
