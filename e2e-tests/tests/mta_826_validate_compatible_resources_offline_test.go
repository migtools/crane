...
var _ = Describe("Crane validate: all compatible standard resources in offline mode", func() {
	It("[MTA-826] Validate final manifests in offline mode using captured API surface as namespace-admin (tier0)", Label("tier0", "validate"), func() {
		...
		// Get the directory containing the crane binary, then go up to repo root
		craneBinDir := filepath.Dir(config.CraneBin)
		repoRoot := filepath.Dir(craneBinDir)
