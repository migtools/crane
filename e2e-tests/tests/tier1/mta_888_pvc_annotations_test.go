	...
	It("[MTA-888] preserves user PVC annotations and lets the target provisioner set binding annotations",
		Label("tier1", "pvc-transfer"), func() {
			const (
				...
				annotationKey   = "e2e.crane.konveyor.io/retention"
				annotationValue = "keep-unchanged"
			)
			...
			srcPVC, err := GetPVC(scenario.KubectlSrc.Context, namespace, pvcName)
			Expect(err).NotTo(HaveOccurred())
			...
			srcPVC, err = GetPVC(scenario.KubectlSrc.Context, namespace, pvcName)
			Expect(err).NotTo(HaveOccurred())
			Expect(srcPVC.Annotations).To(HaveKeyWithValue(annotationKey, annotationValue))
			...
			destPVC, err := GetPVC(scenario.KubectlTgt.Context, namespace, pvcName)
			Expect(err).NotTo(HaveOccurred())
			Expect(destPVC.Status.Phase).To(Equal(corev1.ClaimBound), "destination PVC must bind after the application starts")
			Expect(destPVC.Annotations).To(HaveKeyWithValue(annotationKey, annotationValue),
			...
			log.Printf("Destination PVC %s/%s: StorageClass=%s provisioner=%s", namespace, pvcName, destStorageClass, provisioner)
		})
})
