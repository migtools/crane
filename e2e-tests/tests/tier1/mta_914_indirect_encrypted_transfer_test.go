	...
	It("[MTA-914] Should migrate MySQL PVC data via encrypted S3 and store only ciphertext at rest",
		Label("tier1", "pvc-transfer", "indirect"), func() {
			...
			Expect(DeployVerifierPod(kubectlTgt, VerifierPodOptions{
				...
				Volumes:    []PodVolumeMount{{PVCName: dataPVC, MountPath: "/test-data"}},
				Restricted: true,
			})).NotTo(HaveOccurred())
			...
			const inspectS3Pod = "inspect-s3-bucket-after-transfer"
...
`, inspectS3Pod, namespace, rcloneInspectImage, inspectSecret)
			...
			DeferCleanup(func() {
				if _, err := kubectlTgt.Run("delete", "pod", inspectS3Pod, "-n", namespace, "--ignore-not-found", "--wait=true"); err != nil {
					log.Printf("cleanup inspect pod %q: %v", inspectS3Pod, err)
				}
				if _, err := kubectlTgt.Run("delete", "secret", inspectSecret, "-n", namespace, "--ignore-not-found=true"); err != nil {
					log.Printf("cleanup inspect secret %q: %v", inspectSecret, err)
				}
			})
			_, err = kubectlTgt.Run("wait", "--for=condition=Ready", "pod/"+inspectS3Pod, "-n", namespace, "--timeout=120s")
			Expect(err).NotTo(HaveOccurred())
			...
			rawOut, err := kubectlTgt.Run("exec", inspectS3Pod, "-n", namespace, "--", "/bin/sh", "-c", inspectScript)
